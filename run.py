...
def run_epoch(max_problem, pro_path, batch_size, is_train, min_problem_num, max_problem_num,
    ...
    for i, data in tqdm(enumerate(data_loader), desc='加载中...'):
        ...
        if is_train:
            ...
            predict, state_loss, pro_loss, react_loss = model(last_problem, last_ans, next_problem, next_ans)

            next_predict = torch.masked_select(predict, mask)
            next_true = torch.masked_select(next_ans, mask)

            loss = criterion(next_predict, next_true) + state_loss + pro_loss + react_loss
            loss.backward()
            ...
            with torch.no_grad():
                predict, _, _, _ = model(last_problem, last_ans, next_problem, next_ans)
                next_predict = torch.masked_select(predict, mask)
                next_true = torch.masked_select(next_ans, mask)
